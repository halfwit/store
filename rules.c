...
struct Input
{
	...
	uchar	*s;		/* input string, if from /mnt/store/rules */
...
pushinput(char *name, int fd, uchar *str)
{
	...
	for(in=input; in; in=in->next)
		if(depth++ >= 10)	/* prevent deep C stack in storage and bad include structure */
...
include(char *s)
{
	...
	if(fd<0 && t[0]!='/' && strncmp(t, "./", 2)!=0 && strncmp(t, "../", 3)!=0){
		snprint(buf, sizeof buf, "/sys/lib/store/%s", t);
		t = buf;
		fd = open(t, OREAD);
	}
	if(fd < 0)
