...
char	Enoexist[] =	"store file does not exist";
char	Eisdir[] =		"file is a directory";
char	Ebadmsg[] =	"bad store message format";
char Enosuchport[] ="no such store port";
...
addport(char *port)
{
	...
	for(i=NQID; i<ndir; i++)
		if(strcmp(port, dir[i].name) == 0)
			return;
	if(i == NDIR){
		fprint(2, "storage: too many ports; max %d\n", NDIR);
		return;
	}
	ndir++;
...
startfsys(void)
{
	...
	srvfd = p[0];
	srvclosefd = p[1];
	sprint(srvfile, "/srv/storage.%s.%d", user, getpid());
	if(putenv("storagesrv", srvfile) < 0)
		error("can't write $storagesrv: %r");
	...
	close(p[0]);
	if(mount(p[1], -1, "/mnt/storage", MREPL, "") < 0)
		error("can't mount /mnt/storage: %r");
	close(p[1]);
}
...
{
	fsysrespond(t, buf, "storage: authentication not required");
	return t;
}
...
fsyswrite(Fcall *t, uchar *buf, Fid *f)
{
	...
	case Qsend:
		...
		fsysrespond(t, buf, "no matching store rule");
		return t;
	}
